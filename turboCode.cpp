//本程序可以进行的编译码长度为8+2*n;18+3*n,32+4*n,50+5*n,72+6*n,98+7*n,128+8*n,162+9*n,200+10*n,242+11*n等
#define Ea 0.8862
#include <iostream>
...
void B(void);//计算B函数
void R(void);//计算R函数
char* juanjima(char* str);   //编码函数
char* interleave(char* str); //交织函数
char* decoder(int n);        //译码函数
int* suijichongpai (int a);  //随机排列函数
...
char* interleave(char* str)
{
	...
	s=1;
	  ...
	  array1=suijichongpai(len);//首先获得一行随机数
